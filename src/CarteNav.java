...
public class CarteNav extends Canvas implements MouseListener, MouseMotionListener{
	...
	public void paint(Graphics gc)
	{
		...
		tampon.drawRect(0, 0, this.largeur-1, this.hauteur-1);
                ...
                if (this.clicProcheAvion == false)
                {
                    float vitVent = Math.round(avion.atmosphere.getVitesseVent_ms()*1.943844f + 0.0f);
                    messageInfo = "Vent: " + avion.atmosphere.getDirectionVent() + " ° -- " + vitVent  + "Kt";
                }
	...
	private void dessineAvion(Graphics gc, Avion avion)
	{
		...
		angleVent = avion.getCap() - avion.atmosphere.getDirectionVent();
                
		avionVitesseKt = (int)((avion.getVitesseVp() - (float)(avion.atmosphere.getVitesseVent_ms() * Math.cos(angleVent * (Math.PI)/180)))*1.943844);
		gc.drawString("Vsol:"+ String.valueOf(avionVitesseKt) + "kt  Alt:" + String.valueOf(avionAltFt) + "ft  cap:" + String.valueOf((int)avion.getCap()), centreAvX+8, centreAvY+8);
